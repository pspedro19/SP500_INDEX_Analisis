...
class ContextAdapter(logging.LoggerAdapter):
    """Adaptador para añadir contexto a los logs."""
...
def load_data(
    ...
    try:
        ...
        if df.isnull().any().any():
            logger.warning(f"Los datos contienen valores faltantes. Columnas afectadas: "
...
def split_data(
    ...
    if total_len < (val_size + test_size + min_train_size):
        ...
        logger.warning(f"Datos insuficientes, ajustando: train={total_len-(val_size+test_size)}, "
...
def determine_integration_orders(
    series: pd.Series, 
...
def search_hyperparameters(
    ...
    if config.optimization_method == "auto_arima":
        ...
        _, order, seasonal_order = run_auto_arima(
            y_train, X_train, d, D, seasonal, config
        )
            ...
            if config.n_jobs != 1:
                errors = Parallel(n_jobs=config.n_jobs)(
                    delayed(evaluate_candidate_model)(
                        train_idx, val_idx, y_train, X_train, 
                        p, d, q, seasonal_order, is_sarimax
                    )
        ...
        _, initial_order, initial_seasonal_order = run_auto_arima(
            y_train, X_train, d, D, seasonal, config
        )
        ...
        def objective(trial):
            ...
            if is_sarimax and initial_seasonal_order:
                P_min, P_max = max(0, initial_seasonal_order[0] - 1), min(config.arima_max_P, initial_seasonal_order[0] + 1)
                Q_min, Q_max = max(0, initial_seasonal_order[2] - 1), min(config.arima_max_Q, initial_seasonal_order[2] + 1)
        ...
        best_params = study.best_params
        final_order = (
            best_params.get("p", initial_order[0]),
            d,
            best_params.get("q", initial_order[2])
        )
        ...
        if model_type == "ARIMA":
            ...
            if exog_cols:
                ...
                if X_future.shape[0] != steps or X_future.shape[1] != len(exog_cols):
                    logger.warning(f"Forma incorrecta de X_future: {X_future.shape}, esperado: ({steps}, {len(exog_cols)})")
...
def generate_comprehensive_timeline(
    ...
    if forecast_df is not None and 'forecast' in forecast_df.columns and len(forecast_df) > 0:
        ...
        ax.plot(forecast_index, forecast_df['forecast'], color='red', linestyle='--', 
                label='Pronóstico (21 días)', linewidth=2.0)
        
        # Incluir bandas de confianza si están disponibles
        if 'lower_95' in forecast_df.columns and 'upper_95' in forecast_df.columns:
            ax.fill_between(forecast_index, forecast_df['lower_95'], forecast_df['upper_95'],
                          color='red', alpha=0.2, label='Int. Confianza 95%')
    ...
    try:
            ...
            try:
                if model_type.startswith("SARIMAX"):
                    y_val_pred = final_model.get_prediction(
                        ...
                        exog=df_val[exog_cols] if exog_cols else None
                    ).predicted_mean
                else:
                    y_val_pred = final_model.get_prediction(
                        start=df_val.index[0],
        ...
        if y_val_pred is not None:
            combined_pred = pd.concat([
                pd.Series(y_val_pred, index=df_val.index),
                pd.Series(test_pred, index=df_test.index)
            ])
        else:
        ...
        try:
            ...
            if model_type.startswith("SARIMAX") and exog_cols:
                analyze_coefficient_stability(
                    df, target_col, exog_cols, f"{instrument}_{model_type}", config.output_dir,
            ...
            try:
                ...
                try:
                    ...
                    val_rmse = np.sqrt(mean_squared_error(
                        df_val[target_col][:min_eval_len], 
                        eval_pred[:min_eval_len]
                    ))
                ...
                model_logger.info(f"Longitudes para Test: índices={len(test_indices)}, valores={len(test_real_values)}, predicciones={test_pred_len}")
...
def main():
    ...
    parser.add_argument("--instruments", nargs="+", default=None,
                      help="Lista de instrumentos a procesar")
    parser.add_argument("--method", choices=["auto_arima", "optuna", "hybrid"], default=None,
                      help="Método de optimización de hiperparámetros")
    parser.add_argument("--trials", type=int, default=None,
                      help="Número de trials para optimización (Optuna)")
    parser.add_argument("--config", type=str, default=None,
                      help="Ruta al archivo de configuración YAML")
    parser.add_argument("--jobs", type=int, default=None,
                      help="Número de trabajos paralelos (-1 para usar todos los núcleos)")
    parser.add_argument("--seasonal_period", type=int, default=None,
                      help="Período estacional (días hábiles)")
    ...
    if config.n_jobs != 1 and len(config.instruments) > 1:
        ...
        results_list = Parallel(n_jobs=n_jobs)(
            delayed(process_instrument)(instrument, config) 

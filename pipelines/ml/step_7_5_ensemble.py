...
class GreedyEnsembleRegressor:
    ...
    def fit(self, X, y):
        ...
        self.metrics = {'RMSE': final_rmse, 'MAE': final_mae, 'R2': final_r2, 'Num_Models': len(self.selected_models)}
    ...
    def plot_score_evolution(self, output_path=None):
        ...
        for i, (score, name) in enumerate(zip(self.score_history, self.selected_names)):
            ax.annotate(name, xy=(i + 1, score), xytext=(5, 5), textcoords='offset points')
...
def main():
    ...
    logging.info(
        f"Estadísticas del target: min={df[target_col].min()}, max={df[target_col].max()}, mean={df[target_col].mean()}"
    )

...
def merge_dataframes(
    ...
    logging.info(
        f"Merge completado en {stats['tiempo_merge']:.2f}s: {stats['filas_resultado']} filas, {stats['columnas_resultado']} columnas."
    )

    # Guardar estadísticas
    os.makedirs(CSV_REPORTS, exist_ok=True)
    stats_df = pd.DataFrame([stats])
    write_dataframe(stats_df, os.path.join(CSV_REPORTS, 'merge_stats.csv'))
...
def main() -> bool:
    ...
    for archivo in archivos:
        ...
        df = read_dataframe(ruta_completa)
    ...
    logging.info(
        f"Combinación exitosa. Resultado tiene {len(df_combinado)} filas y {len(df_combinado.columns)} columnas"
    )
    ...
    try:
        t0 = time.perf_counter()
        write_dataframe(df_combinado, archivo_salida)
        t1 = time.perf_counter()
        ...
        write_dataframe(stats_df, os.path.join(CSV_REPORTS, "merge_complete_stats.csv"))
        return True

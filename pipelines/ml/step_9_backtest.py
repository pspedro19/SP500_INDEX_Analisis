...
def calcular_metricas_por_subperiodos(
    df, real_col='Valor_Real', pred_col='Valor_Predicho', date_col=DATE_COL, freq='Q'
):
    """
    ...
    for modelo in df['Modelo'].unique():
        ...
        for periodo, grupo in df_modelo.groupby('periodo'):
            ...
            resultado = {'Modelo': modelo, 'Periodo': periodo, 'Num_Observaciones': len(grupo), **metricas}
...
def main():
    ...
    try:
        ...
        for (modelo, mercado), grupo in combinaciones:
            ...
            resultado = {'Tipo_de_Mercado': mercado, 'Modelo': modelo, **metricas}
            ...
            for freq, freq_name in [('Q', 'trimestre'), ('A', 'año')]:
                df_subperiodos, periodo_label = calcular_metricas_por_subperiodos(grupo, freq=freq)

                if not df_subperiodos.empty:
                    # Guardar CSV de subperíodos
                    subperiod_file = os.path.join(
                        subperiods_dir, f"{mercado_filename}_{modelo_filename}_por_{freq_name}.csv"
                    )
                    ...
                    for metric in ['RMSE', 'MAE', 'Hit_Direction']:
                        if metric in df_subperiodos.columns:
                            chart_path = os.path.join(
                                subperiods_dir, f"{mercado_filename}_{modelo_filename}_{metric.lower()}_evolution.png"
                            )

                            plt.figure(figsize=(10, 6))
                            plt.plot(df_subperiodos['Periodo'], df_subperiodos[metric], marker='o', linestyle='-')
                    ...
                    dashboard_path = os.path.join(
                        subperiods_dir, f"{mercado_filename}_{modelo_filename}_metrics_dashboard.png"
                    )
                    ...
                    for metric, title, ax in metrics_to_plot:
                        if metric in df_subperiodos.columns:
                            ax.plot(df_subperiodos['Periodo'], df_subperiodos[metric], marker='o', linestyle='-')
                            ax.set_title(title)
        ...
        if resultados:
            ...
            for mercado in mercados:
                ...
                if len(metrics_dict) > 1:  # Solo si hay más de un modelo
                    ...
                    plot_radar_metrics(
                        metrics_dict, title=f"Comparación de métricas - {mercado}", output_path=radar_path
                    )
            ...
            for metric in main_metrics:
                if metric in df_resultado.columns:
                    for mercado in mercados:
                        ...
                        chart_path = os.path.join(
                            output_charts, f"{mercado.replace(' ', '_')}_{metric.lower()}_comparison.png"
                        )
                        plt.figure(figsize=(10, 6))

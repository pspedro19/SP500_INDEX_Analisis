...
logger = setup_logging(settings.log_dir, "combine_predictions")
...
def parse_arguments():
    ...
    parser.add_argument('--target_col', type=str, required=True, help='Nombre de la columna objetivo (target).')
    ...
    parser.add_argument(
        '--ml_weight', type=float, default=0.5, help='Peso para las predicciones ML (0-1) en método weighted o custom.'
    )

    parser.add_argument(
        '--weights_file', type=str, default=None, help='Archivo JSON con pesos personalizados para cada modelo.'
    )

    parser.add_argument(
        '--calc_weights_from_valid', action='store_true', help='Calcular pesos basados en el rendimiento en validación.'
    )
    ...
    parser.add_argument('--ensemble_name', type=str, default='Ensemble', help='Nombre para el modelo ensamblado.')

    parser.add_argument(
        '--date_column', type=str, default='date', help='Nombre de la columna de fecha en los archivos de entrada.'
    )

    parser.add_argument(
        '--drop_duplicates', action='store_true', help='Eliminar predicciones duplicadas para la misma fecha.'
    )
...
def identify_forecast_columns(df, target_col, model_type=None):
    ...
    column_info = {'forecast': None, 'lower': None, 'upper': None, 'actual': None, 'model_type': model_type}
...
def extract_model_predictions(df, target_col, model_types=None):
    ...
    if not model_types:
        ...
        if model_col:
            ...
            ml_pattern = any(
                ('ml' in col.lower() or 'xgboost' in col.lower() or 'lightgbm' in col.lower()) for col in df.columns
            )
...
def calculate_weights_from_validation(aligned_models, validation_periods=21):
    ...
    for model_type, model_df in aligned_models.items():
        ...
        # Guardar errores
        errors[model_type] = {'mae': mae, 'rmse': rmse}
...
def format_for_powerbi(models_dict, ensemble_df, target_col, include_all_models=True, ensemble_name='Ensemble'):
    ...
    if ensemble_df is not None:
        ...
        ensemble_copy = ensemble_copy.rename(
            columns={k: v for k, v in column_mapping.items() if k in ensemble_copy.columns}
        )
    ...
    if include_all_models:
        for model_type, model_df in models_dict.items():
            ...
            model_copy = model_copy.rename(columns={k: v for k, v in column_mapping.items() if k in model_copy.columns})
...
def combine_predictions(
    ...
    ml_weight=0.5,
    weights_file=None,
    calc_weights_from_valid=False,
    validation_periods=21,
    ...
    try:
        ...
        write_dataframe(output_df, output_file)

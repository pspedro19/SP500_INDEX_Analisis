        ...
        if historical_data_path.parent.exists():
            ...
            if not all_predictions.empty:
                ...
                hist_data = all_predictions[
                    (all_predictions['Tipo_Mercado'] == instrument) & 
                    (all_predictions['Periodo'] != 'Forecast')
                ].copy()
        ...
        if 'Valor_Real' in model_data.columns and 'Valor_Predicho' in model_data.columns:
            ...
            try:
                ...
                if len(real) > 0:
                    ...
                    logger.info(f"Métricas para {instrument}_{model_type}: RMSE={rmse:.4f}, MAE={mae:.4f}, Hit={hit_dir:.2f}%")
            except Exception as e:
    ...
    try:
        ...
        for model, metrics in metrics_comparison.items():
            metrics_by_model[model] = {
                key.capitalize(): value for key, value in metrics.items()
            }
...
def visualize_ensemble_forecasts(
    ...
    try:
        ...
        for model_name, forecast_df in models_forecasts.items():
            if 'forecast' in forecast_df.columns:
                forecast_series[model_name] = pd.Series(
                    ...
                    index=forecast_df.index if isinstance(forecast_df.index, pd.DatetimeIndex) 
        ...
        ensemble_series = pd.Series(
            ...
            index=ensemble_forecast.index if isinstance(ensemble_forecast.index, pd.DatetimeIndex)
                ...
                if historical_data is None or historical_data.empty:
                    ...
                    if historical_data.empty:
                        ...
                        historical_data = historical_data.pivot(
                            index='date', 
                            columns='Modelo', 
                            values=['Valor_Real', 'Valor_Predicho']
                        )
